...
```{r}
cust_db_binom <- merge(booktrain, book, by.x = "id", by.y = "id")
...
fit <- glm(logtargamt.x ~ recency, data = cust_db_binom, family = binomial)
summary(fit)
plot(fit)

plot(cust_db_binom$recency, cust_db_binom$logtargamt.x)



cust_db_binom$prediction <- predict(fit, cust_db_binom, type = "response")

tab = table(cust_db_binom$logtargamt.x, cust_db_binom$prediction > .04)
tab

mean(cust_db_binom[cust_db_binom$logtargamt.x == 1,]$prediction)
mean(cust_db_binom[cust_db_binom$logtargamt.x == 0,]$prediction)

summary(cust_db_binom$recency)
